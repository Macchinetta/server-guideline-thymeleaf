...
.. warning:: **リクエストパラメータの存在チェックについて**
    ...
    \ ``org.thymeleaf.context.WebEngineContext.RequestParametersMap`` \ の実装により、\ ``param.containsKey``\  は\ `一律true <https://github.com/thymeleaf/thymeleaf/blob/thymeleaf-3.0.15.RELEASE/src/main/java/org/thymeleaf/context/WebEngineContext.java#L567-L574>`_\ が返却されるため、\ ``param.keySet().contains``\ を使用してパラメータの有無を判断する必要がある。
...
\ なお、`ブランクプロジェクト <https://github.com/Macchinetta/macchinetta-web-multi-blank-thymeleaf/tree/1.8.3.RELEASE>`_\ には\ ``AuthenticationPrincipalArgumentResolver``\ が設定済みである。
